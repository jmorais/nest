<?php
// AvianVisitors - proxies audio clips from BirdNET-Go's API.
//
// Usage:
//   /avian/api/recording.php?sci=<scientific name>
//   /avian/api/recording.php?file=<detection id>
//   /avian/api/recording.php?file=<filename>
//
// Modern BirdNET-Go audio endpoint:
//   GET /api/v2/audio/<detection_id>
//
// Filename fallback endpoint:
...
// file can be either a numeric detection ID or a legacy clip filename.
if ($file !== '') {
  $is_detection_id = preg_match('/^\d+$/', $file);
  $is_audio_file = preg_match('/^[a-z0-9_.-]+\.(wav|mp3|flac|aac|opus|ogg)$/i', $file);

  if (!$is_detection_id && !$is_audio_file) {
    http_response_code(400);
    header('Content-Type: text/plain; charset=utf-8');
...
/**
 * Try to choose the best detection from a list.
 * Current behavior: highest confidence for the requested species.
 */
...
/**
 * Send a small JSON error response and exit.
 */
...
// Mode 1: file parameter.
// This can now be a BirdNET-Go detection ID, e.g. ?file=7020.
if ($file !== '') {
  if (preg_match('/^\d+$/', $file)) {
    $audio_url = $BIRDNET_GO_URL . '/api/v2/audio/' . rawurlencode($file);
  } else {
    $audio_url = $BIRDNET_GO_URL . '/api/v2/media/audio/' . rawurlencode($file);
  }
} else {
  ...
  // If that does not work, try recent detections and filter locally.
  if ($raw === null) {
  ...
  $det_list = extract_detection_list($raw);
  $audio_id = find_best_detection_id($det_list, $sci);

  if ($audio_id === null) {
    json_error(404, [
      'error' => 'no recording found for ' . $sci,
      'hint'  => 'add &debug=1 to the URL to inspect the BirdNET-Go response',
    ]);
  }

  $audio_url = $BIRDNET_GO_URL . '/api/v2/audio/' . rawurlencode($audio_id);
}
...
// ---------------------------------------------------------------------------
// Proxy the audio stream from BirdNET-Go
// ---------------------------------------------------------------------------
...
$upstream = @fopen(
  ...
  'rb',
  false,
  bg_context($HTTP_TIMEOUT, '*/*')
);

if ($upstream === false) {
  http_response_code(502);
  header('Content-Type: text/plain; charset=utf-8');
  echo 'could not connect to BirdNET-Go audio endpoint';
  exit;
}
...
if ($status_code === 404) {
  ...
  header('Content-Type: text/plain; charset=utf-8');
  echo 'clip not found on BirdNET-Go';
  exit;
}
...
if ($content_type === null || stripos($content_type, 'audio/') !== 0) {
